CREATE DATABASE IF NOT EXISTS regime CHARACTER
SET
...
CREATE TABLE
	client_objectifs (
		...
		CONSTRAINT fk_client_objectifs_client FOREIGN KEY (client_id) REFERENCES users (id) ON UPDATE CASCADE ON DELETE CASCADE,
		CONSTRAINT fk_client_objectifs_objectif FOREIGN KEY (objectif_id) REFERENCES objectifs (id) ON UPDATE CASCADE ON DELETE RESTRICT
	);
...
CREATE TABLE
	regimes (
		...
		CONSTRAINT chk_total_prct CHECK (
			pourcentage_poisson + pourcentage_volaille + pourcentage_viande <= 100
		),
...
CREATE TABLE
	regime_sports (
		...
		CONSTRAINT fk_regime_sports_regime FOREIGN KEY (regime_id) REFERENCES regimes (id) ON UPDATE CASCADE ON DELETE CASCADE,
		CONSTRAINT fk_regime_sports_sport FOREIGN KEY (sport_id) REFERENCES sports (id) ON UPDATE CASCADE ON DELETE RESTRICT,
		CONSTRAINT fk_regime_sports_client_objectif FOREIGN KEY (client_id, objectif_id, date_choix) REFERENCES client_objectifs (client_id, objectif_id, date_choix) ON UPDATE CASCADE ON DELETE CASCADE,
...
CREATE TABLE
	client_options (
		...
		CONSTRAINT fk_client_options_client FOREIGN KEY (client_id) REFERENCES users (id) ON UPDATE CASCADE ON DELETE CASCADE,
		CONSTRAINT fk_client_options_option FOREIGN KEY (option_id) REFERENCES options (id) ON UPDATE CASCADE ON DELETE RESTRICT
	);
...
CREATE TABLE
	mvt_compte (
		...
		CONSTRAINT fk_mvt_compte_client FOREIGN KEY (client_id) REFERENCES users (id) ON UPDATE CASCADE ON DELETE CASCADE,
